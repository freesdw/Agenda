...
Date::Date(const string& dateString) {
    ...
    if(valid)
        sscanf(dateString, "%d-%d-%d/%d:%d", &m_year, &m_month, &m_day, &m_hour, &m_minute);
    else *this = Date();
}
...
bool Date::isValid(const Date& t_date) {
    if(t_date.m_year < 1000 || t_date.m_year > 9999)
        return false;
    if(t_date.m_month < 1 || t_date.m_month > 12 || t_date.day < 1)
        return false;
...
string Date::dateToString(const Date& t_date) {
    char str[17] = {0};
    sprintf(str, "%4d-%2d-%2d/%2d:%2d", t_date.m_year, t_date.m_month, t_date.m_day, t_date.m_hour, t_date.m_minute);
    if(!isValid(t_date))
